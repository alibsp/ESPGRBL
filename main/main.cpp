...
static void wifi_setup() {
    ...
    memcpy(wifi_config.ap.ssid,AP_SSID_IN,strlen(AP_SSID_IN));
    memcpy(wifi_config.ap.password,AP_PSSWD_IN,strlen(AP_PSSWD_IN));

    if (strlen(AP_PSSWD_IN) == 0) {
...
void debugTask (void *pvParameters)
{
    ...
    while(1)
    {
        ...
        printf("st.exec_block->direction_bits1 = %d\n",st.exec_block->direction_bits);
    }
}
